...
const input_organizacion = document.querySelector('#txt-organizacion');
const input_exp = document.querySelector('#txt-exp');
...
const tipo_usuario = 'organizador';

const btn_crear_cuenta = document.querySelector('#btn_registrar');

let validar = (pnombre, pgenero, porganizacion, pexp, pcorreo, pid, pprimer_apellido, psegundo_apellido, pfecha, pprovincia, pcanton, pdistrito, pdireccion_exacta) => {
    ...
    if (porganizacion == '') {
        error = true;
        input_organizacion.classList.add('input_error');
    } else {
        input_organizacion.classList.remove('input_error');
    }

    if (pexp == '') {
        error = true;
        input_exp.classList.add('input_error');
    } else {
        input_exp.classList.remove('input_error');
    }
...
let llamar = async () => {
    ...
    let organizacion = input_organizacion.value;
    let exp = input_exp.value;
    ...
    let error = validar(nombre, genero, organizacion, exp, correo, id, primer_apellido, segundo_apellido, fecha, provincia, canton, distrito, direccion_exacta);
    ...
    if (edad < 18) {
        ...
        if (error == false && errorCedula == false && errorCorreo == false) {
            registrarOrganizador(src_avatar, nombre, snombre, genero, organizacion, exp, correo, contrasena, id, primer_apellido, segundo_apellido, fecha, direccion_exacta, provincia, canton, distrito, tipo_usuario);
            Swal.fire({ //formato json

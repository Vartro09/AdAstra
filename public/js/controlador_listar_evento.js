...
let mostrar_tabla = async () => {
    ...
    for (let i = 0; i < lista_eventos.length; i++) {
        ...
        fila.insertCell().innerHTML = lista_eventos[i]['nombre'];
        fila.insertCell().innerHTML = lista_eventos[i]['encargado'];
        fila.insertCell().innerHTML = lista_eventos[i]['recinto'];
        ...
        iIconEditar.addEventListener('click', function () {
            window.location.href = `modificar_evento.html?_id=${lista_eventos[i]['_id']}`;
        });
        //////////////////////////////////////////////////////////////////////////////////////
        ...
        iIconEliminar.addEventListener('click', function () {
            Swal.fire({
                ...
                if (result.value) {
                    eliminarEvento(lista_eventos[i]._id);
                    Swal.fire(
                        'Evento eliminado!'
                    ).then((result) => {
                        if (result.value) {
...
let filtrar_tabla = async () => {
    ...
    for (let i = 0; i < lista_eventos.length; i++) {
        if (lista_eventos[i]['nombre'].toLowerCase().includes(filtro) || lista_eventos[i]['encargado'].toLowerCase().includes(filtro) || lista_eventos[i]['recinto'].toLowerCase().includes(filtro) || lista_eventos[i]['tipo_evento'].toLowerCase().includes(filtro)) {
            let fila = tbody.insertRow();
            fila.insertCell().innerHTML = lista_eventos[i]['nombre'];
            fila.insertCell().innerHTML = lista_eventos[i]['encargado'];
            fila.insertCell().innerHTML = lista_eventos[i]['recinto'];

...
let llamar = async () => {
    ...
    if (edad < 18) {
        ...
        if (error == false && errorCedula == false && errorCorreo == false) {
            registrarCliente(src_avatar, nombre, snombre, genero, correo, contrasena, id, primer_apellido, segundo_apellido, fecha, direccion_exacta, provincia, canton, distrito, tipo_usuario);
            Swal.fire({ //formato json
